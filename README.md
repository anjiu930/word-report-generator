...
一个 **Skill 插件** —— 一句话生成格式完美的 Word 课程设计报告。

**建议在 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 中使用，也支持其他兼容 Skill 的环境。**
...
> 让 ChatGPT 帮你写参考文献，结果一查——全是编造的期刊、不存在的作者、虚构的页码……老师一眼就看出来了。

**🚫 还在让 AI 生成报告然后反复 Ctrl+C / Ctrl+V 吗？**

> 从 AI 对话框里一段段复制，粘贴到 Word 里，然后手动调字体、改行距、设缩进、排封面、做目录……报告内容可能十分钟就写完了，排版却耗掉了你一整个下午。

如果你是理工科大学生，你一定还经历过这些：
...
| | 以前手写报告 | 用这个工具 |
|---|---|---|
| 写作方式 | AI 生成 → 反复 Ctrl+C/V → 手动调格式 | 💬 一句话，直接生成完整 Word |
| 排版格式 | 每次手动调字体、行距、缩进 | ✨ 全自动，严格符合规范 |
| 参考文献 | AI 编造假文献，查都查不到 | 🔍 知网实时搜索，真实可查 |
| 写作时间 | 8~15 小时 | ⏱️ 5 分钟生成初稿 |
| 代码展示 | 截图粘贴 / 手动排版代码 | 💻 自动从源码提取 + 排版 |
| 图片引用 | 经常忘记标注"如图X所示" | 🖼️ 自动生成引用 + 占位符 |
| 跨课程复用 | 每次从零开始 | 🔄 换个模板就能复用 |

## 🖼️ 效果预览

> 以下均为工具实际生成的报告截图。生成后你只需要替换图片占位符为真实截图即可提交。
...
<details>
<summary><b>⚠️ 报错 "No module named 'docx'"</b></summary>
